...
# 需要 period 的指标
_PERIOD_INDS = {"MA", "EMA", "MA_VOL", "RSI", "BOLL"}
# 子字段指标（用 field 指定分量）
...
def dsl_to_text(dsl: StrategyDSL) -> str:
    ...
    def ind_text(ind: Indicator) -> str:
        ...
        if n in _PATTERNS:
            ...
            return f"{n2}" + (f"({ind.period})" if ind.period else "")
        if n in _FIELD_INDS:
            return {"CLOSE": "收盘价", "OPEN": "开盘价", "HIGH": "最高价", "LOW": "最低价",
        ...
        return "".join(parts) if n in ("MA", "EMA", "MA_VOL", "RSI") else f"{base}({ind.field or ''}{ind.period or ''})"
